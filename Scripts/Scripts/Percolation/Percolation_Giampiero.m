...
%%
load('C:\Users\Windows\OneDrive - Università di Pavia\Desktop\Roba\Work\ARMAmodel\ARMAModel\Script\Giampiero\Wakefulness_New.mat')
%%
...
%%
load('C:\Users\Windows\OneDrive - Università di Pavia\Desktop\Roba\Work\ARMAmodel\ARMAModel\Script\Giampiero\Anaesthesia_New.mat')
%%
Ch = setdiff(1:192,[66,20,88,174,113,131,133]);
...
%%
load('C:\Users\Windows\OneDrive - Università di Pavia\Desktop\Roba\Work\ARMAmodel\ARMAModel\Script\Giampiero\Awakening_New.mat')
%%
Ch = setdiff(1:192,[66,20,88,174,113,131,133]);
...
%%
% load('C:\Users\sofia\OneDrive - Istituto Superiore di Sanità\ARMAmodel\ARMAModel\Script\Giampiero\NuoviDati\Wakefulness2_Bart.mat')
% %%
% Ch = setdiff(1:192,[66,20,88,174,113,131,133]);
% ind = round(size(LFP_W2,2)/16)-1;
% %%
% for i=1:16
%     ndxT = 1+ind*(i-1):ind+ind*(i-1);
%     Matrix(:,:,i) = corr(MUA_W2(Ch,ndxT)',MUA_W2(Ch,ndxT)');
%     [perc_threshold,Veglia_finale.Perc_Matrix(:,:,i), Veglia_finale.Sort_value{i},Veglia_finale.n_com_size{i},perc_threshold_step] = Percolation(Matrix(:,:,i));
% end
% Veglia_finale.Perc_Matrix = mean(Veglia_finale.Perc_Matrix,3);
% %%
% Veglia_finale.Perc_Matrix_pruned =[];
% for i=1:size(Veglia_finale.Perc_Matrix,1)
%     for j=1:size(Veglia_finale.Perc_Matrix,2)
%         if Veglia_finale.Perc_Matrix(i,j)<0.4
%             Veglia_finale.Perc_Matrix_pruned(i,j)=0;
%         else
%             Veglia_finale.Perc_Matrix_pruned(i,j)=Veglia_finale.Perc_Matrix(i,j);
%         end
%     end
% end
% %%
% d  = graph(Veglia_finale.Perc_Matrix_pruned,'upper');
% figure;
% h = plot(d,'LineStyle','--','MarkerSize',5, 'NodeColor', 'g');
% highlight(h, 1:93, 'NodeColor', 'r');
% %%
% figure;
% imagesc(Veglia_finale.Perc_Matrix_pruned)
%%
figure
for i=1:16
...
% veglia = {};
% for i=1:16
%     ind = find(Veglia.Sort_value{i}>0.2 & Veglia.Sort_value{i}<0.6);
%     nd = find(Veglia.n_com_size{i}(ind)>60 & Veglia.n_com_size{i}(ind)<120);
%     veglia{i} = Veglia.n_com_size{i}(ind(nd));
% end
%%
cross_veglia =[];
cross_an =[];
...
%
% for i=1:16
%     ndxT = 1+1*(i-1):ind+1*(i-1);
%     Matrix(:,:,i) = corr(MUA_veglia(Ch,ndxT)',MUA_veglia(Ch,ndxT)');
% end
% Matrix = mean(Matrix,3);
% %%
% [perc_threshold,Perc_Matrix, Sort_value,n_com_size,perc_threshold_step] = Percolation(Matrix);
%