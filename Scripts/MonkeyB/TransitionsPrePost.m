% Script to analyze the Up-to-Down transitions in Monkey B.
% This script reproduces figure 6 of the paper.
clear all;
%%
...
%%
load('path\to\B\KernelAndPerfPF_trans4000.mat')
% Out_PF = OutReconstructTest;
for i=1:96
    Out_PF(i,:) = reshape(Kernel(:,:,i)',1,69*numel(ChPF))*InputTest_PF;
    % Out_PF(i,:) = reshape(Kernel(:,:,i)',1,69*numel(ChPF))*InputTest_PM;
end

load('path\to\B\KernelAndPerfPM_trans4000.mat')
% Out_PM = OutReconstructTest;
for i=1:96
    Out_PM(i,:) = reshape(Kernel(:,:,i)',1,69*numel(ChPM))*InputTest_PM;
    % Out_PM(i,:) = reshape(Kernel(:,:,i)',1,69*93)*InputTest_PF;
end
%%
lpCutOffFreq = 12.5;
for nc = 1:numel(ChPF)
