% Script to train the AR model on PMc and testing on PFc for the Monkey B dataset.
% Changing the transient period to 100 (Wa.), 3000 (An.), 7600 (Aw.).
% This script reproduces figure 5a of the paper.
%%
clear all;
%%
%%
load('path\to\B\MEAMUALFP.mat')
load('path\to\B\PreProcData.mat')
%%
AllChIn =97:192;
AllChOut = 1:96;
ChIn = setdiff(97:192,[174,113,131,133]);
ChOut = setdiff(1:96,[66,20,88]);
...
%%    
save('KernelAndPerfPM2PF_trans7600.mat', 'Kernel','PerformanceTraining','OutReconstructTraining','PerformanceTest','PvalueTest','OutReconstructTest','OutOrig','OutOrigTest');
%%
...
%         MeasureMap(ndxCh2Map(LC.channels-96)) = PerformanceTest;
        MeasureMap(ndxCh2Map(AllChOut)) = PerformanceTest;
        imagesc(MeasureMap);
        colorbar();
%         colormap(BlueRedCM);
        hold on
